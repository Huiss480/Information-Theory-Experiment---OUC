...
def calcShannonEnt(dataSet):
    ...
    for featVec in dataSet:  # 对每组特征向量进行统计
        ...
        labelCounts[currentLabel] += 1  # 如果标签(Label)已经放入统计次数的字典进行 Label计数+1
...
def splitDataSet(dataSet, axis, value):
    ...
    for featVec in dataSet:  # 遍历数据集
        if featVec[axis] == value:
            ...
            retDataSet.append(reducedFeatVec) # 加入到划分后的数据集
...
def chooseBestFeatureToSplit(dataSet):
    ...
    baseEntropy = calcShannonEnt(dataSet)  # 计算数据集的香农熵，第一步
    bestInfoGain = 0.0  # 信息增益
    bestFeature = -1  # 最优特征的索引值
    for i in range(numFeatures):  # 征遍历所有特征
        # 获取dataSet的第i个所有特征
        featList = [example[i] for example in dataSet]
        uniqueVals = set(featList)  # 创建set集合{},元素不可重复，获取第一个属性的所有取值
...
Parameters:
    dataSet - 训练数据集
    labels - 分类属性标签
    featLabels - 主函数定义的存储选择的最优特征标签的变量
Returns:
    myTree - 决策树
"""
